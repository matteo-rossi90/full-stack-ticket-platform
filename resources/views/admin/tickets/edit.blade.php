        ...
        <div class="card p-4">
            ...
            <form action="{{route('admin.tickets.update', $ticket)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="title" name="title" value="{{old('title', $ticket->title)}}" placeholder="Es: Rallentamento sistema">
                    @error('title')
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" name="date" id="date" value="{{old('date', $ticket->date)}}">
                    @error('date')
                        <small class="text-danger">{{$message}}</small>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                   ...
                   <select class="form-select" id="operator_id" name="operator_id">
                    <option value="">Seleziona operatore</option>
                        ...
                        @if(old('operator_id', $ticket->operator->id) == $operator->id) selected @endif>{{$operator->name}} {{$operator->surname}}</option>
                        @endforeach
                    </select>
                    @error('operator_id')
                        <small class="text-danger">{{$message}}</small>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="category_id" name="category_id">
                        ...
                        @if(old('category_id', $ticket->category->id) == $category->id) selected @endif>{{$category->name}}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <small class="text-danger">{{$message}}</small>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="message" name="message" value="{{old('message', $ticket->message)}}">{{$ticket->message}}</textarea>
                    @error('message')
                        <small class="text-danger">{{$message}}</small>
                    @enderror
                </div>
